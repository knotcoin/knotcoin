Knotcoin Core version 0.16.0 is now available from:

  <https://knotcoincore.org/bin/knotcoin-core-0.16.0/>
...
installer (on Windows) or just copy over `/Applications/Knotcoin-Qt` (on Mac)
or `knotcoind`/`knotcoin-qt` (on Linux).
...
Knotcoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
knotcoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Knotcoin Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µKNOT" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- knotcoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- knotcoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 17449 to avoid conflict with testnet's default of 17338.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest knotcoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.knotcoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for knotcoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to knotcoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #10825 `28485c7` Set regtest JSON-RPC port to 17449 to avoid conflict with testnet 17338 (fametrano)
...
- #12035 `eeb6d52` Change µKNOT to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_knotcoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` knotcoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.knotcoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for knotcoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in knotcoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop knotcoind (OmeGak)
...
- #10529 `331352f` Improve knotcoind systemd service file (Flowdalic)
