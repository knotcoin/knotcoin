...
The RPM spec file provided here is for Knotcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://knotcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Knotcoin git repository](https://github.com/knotcoin/knotcoin) but are not part of
...
Currently only `Source10` is used, for the example `knotcoin.conf` file.
...
in the knotcoin git repository. At present only an SVG version of the Knotcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the knotcoin
...
needed when 0.12.1 is released, a proper fix is already in the Knotcoin git
master branch.
...
idea to build Knotcoin Core against a static version of that library compiled
...
The `knotcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `knotcoin-server` package. At this point, most RPM based
...
The `knotcoin-server` package should have SELinux support. How to properly do
...
Knotcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/knotcoin/). He has been
packaging Knotcoin for Fedora at least since 2012.
...
4. I renamed the `knotcoin` package that contains the Qt GUI to `knotcoin-core` as
that appears to be how the general population refers to it, in contrast to
`knotcoin-xt` or `knotcoin-classic`. I wanted to make sure the general population
