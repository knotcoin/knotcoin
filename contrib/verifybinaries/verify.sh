...
###   knotcoincore.org and knotcoin.org and compares them.
...
WORKINGDIR="/tmp/knotcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://knotcoincore.org"
HOST2="https://knotcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="knotcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'knotcoin-',
...
if [ $? -ne 0 ]; then
   echo "knotcoin.org failed to provide signature file, but knotcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "knotcoin.org and knotcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Knotcoin Core binary release signing key installed?"
   fi
