#!/bin/bash
# Copyright (c) 2013 The Knotcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Knotcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Knotcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Knotcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Knotcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Knotcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Knotcoin 0.8 datadir."
    ;;
esac
