...
{
    QString ret = QString("knotcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non KNOT unit input in lnReqAmount, we generate the URI with KNOT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(KnotcoinUnits::format(KnotcoinUnits::KNOT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
